...
M_w = nan(size(tspan));  % O in sedimentary rocks
M_LT = nan(size(tspan));   % O in upper crust
M_HT = nan(size(tspan)); % O in deep crust
...
for i = 2:nt
    ...
    F_mw(i) = phi_w(i)*f_Krc(i) * F_mw_0;   % Formation rate of continental sediments
    F_sw(i) = phi_w(i)*f_Krc(i) * F_LT_0 + phi_w(i)*(1 - r_w(i)) * (F_w(i) + F_reverse(i));   % subduction rate of sediments    
    F_mLT(i) = phi_LT(i)*f_LT(i) * F_mLT_0;   % Formation rate of low temperature altered OC
    F_sLT(i) = phi_LT(i)*f_LT(i) * F_LT_0 + phi_LT(i)*(1 - r_LT(i)) * F_LT(i);   % subduction rate of low temperature altered OC
    F_mHT(i) = phi_HT(i)*f_HT(i) * F_mHT_0;   % Formation rate of high temperature altered OC
    F_sHT(i) = phi_HT(i)*f_HT(i) * F_HT_0 + phi_HT(i)* (1 - r_HT(i)) * F_HT(i);   % subduction rate of high temperature altered OC
    ...
    dM_HT18 = (F_HT18(i) + F_mHT18(i) - F_reHT18(i) - F_sHT18(i) - F_HT_ex(i))*dt;
