function [M_f, M_f18, M_w, M_w18, M_LT, M_LT18, M_HT, M_HT18,...
    ...
    s_model_latest(y_ini, tspan, f_LT,f_LT_Krc,f_HT, F_w_0, F_LT_0, F_HT_0, r_w,...
    r_LT, r_HT, alpha_w, alpha_LT, alpha_HT, alpha_rev,R_si, k_w, ...
    R_oc, k_LT, k_HT, F_pw18_0, phi_m, phi_si, F_mw_0, phi_oc, ...
...
for i = 2:nt
    ...
    omega_w(i)  = (R_si / R_f(i) ) / alpha_w;% isotope saturation index for 18O exchange in silicate weathering
    ...
    F_m(i) = f_HT(i) * F_m_0;  % H2O release from mantle magmatism
    F_LT(i) = f_LT(i) * F_LT_0 ;   % H2O uptake in low T altered OC
    F_HT(i) = f_HT(i) * F_HT_0 ;  % H2O uptake in high T altered OC
    F_w(i) =  f_LT(i) * F_w_0 ;  % H2O fixation in weathering products
    ...
    F_mw(i) = f_LT_Krc(i) * F_mw_0;   % Formation rate of continental sediments
    ...
    % formation of 18O
    F_mw18(i) = phi_si * F_mw(i);   % formation of sedimentary rock 18O
    ...
    F_reverse_ex(i) = f_reverse(i) * f_LT(i) * k_w * (omega_reverse(i) - 1);
